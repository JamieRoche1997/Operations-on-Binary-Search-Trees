...
public class myBinarySearchTreeImpl<T1 extends Comparable<? super T1>, T2> implements myBinarySearchTree<T1, T2> {
	...
	public int my_count_at_level(int level){
		...
		// if the level is 0, return 1 (there's only the root node at level 0)
		if (level == 0) {
			return 1;
		}
		// initialize the count of nodes at the specified level to 0
		int count = 0;

		// recursively count the nodes at the specified level in the left subtree of the root
		if (!this.my_left_tree().my_is_empty()) {
			count += this.my_left_tree().my_count_at_level(level - 1);
		}
		// recursively count the nodes at the specified level in the right subtree of the root
		if (!this.my_right_tree().my_is_empty()) {
			count += this.my_right_tree().my_count_at_level(level - 1);
		}
		// return the total count of nodes at the specified level
		return count;
	}
	...
	public boolean my_is_balanced(){
		...
		// calculate the height of the left and right subtrees of the root
		int leftHeight = this.my_left_tree().my_length();
		int rightHeight = this.my_right_tree().my_length();
		// check if the difference between the heights of the left and right subtrees is less than or equal to 1, and if the left and right subtrees are also balanced
		return Math.abs(leftHeight - rightHeight) <= 1 && this.my_left_tree().my_is_balanced() && this.my_right_tree().my_is_balanced();
	}
	...
	public int my_count_smaller_nodes(T1 key){
		...
		// initialize the count of nodes smaller than the specified key to 0
		int count = 0;
		// if the specified key is greater than the key of the root node, increment the count and recursively count the nodes smaller than the key in the left and right subtrees of the root
		if (key.compareTo(this.my_root().getKey()) > 0) {
			count++;
			if (!this.my_left_tree().my_is_empty()) {
				count += this.my_left_tree().my_count_smaller_nodes(key);
			}
			if (!this.my_right_tree().my_is_empty()) {
				count += this.my_right_tree().my_count_smaller_nodes(key);
			}
		} else { // if the specified key is less than or equal to the key of the root node, recursively count the nodes smaller than the key in the left subtree of the root
			if (!this.my_left_tree().my_is_empty()) {
				count += this.my_left_tree().my_count_smaller_nodes(key);
			}
		}
		// return the total count of nodes smaller than the specified key
		return count;
	}
	...
	public int my_find_node_at_level(T1 key){
		...
		// If the binary search tree is empty, return -1
		if (this.my_is_empty()) {
			return -1;
		}
		// If the key is at the root, return 0
		if (this.my_root().getKey().equals(key)) {
			return 0;
		}
		// Recursively search the left subtree for the key and add 1 to the level if found
		int leftLevel = this.my_left_tree().my_find_node_at_level(key);
		if (leftLevel != -1) {
			return leftLevel + 1;
		}
		// Recursively search the right subtree for the key and add 1 to the level if found
		int rightLevel = this.my_right_tree().my_find_node_at_level(key);
		if (rightLevel != -1) {
			return rightLevel + 1;
		}
